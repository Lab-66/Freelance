<div class="panel panel-primary">
    <div class="panel-body">
        @if (isset($invoice))
            {!! Form::model($invoice, array('url' => $type . '/' . $invoice->id, 'method' => 'put', 'files'=> true, 'id'=>'form')) !!}
        @else
            {!! Form::open(array('url' => $type, 'method' => 'post', 'files'=> true, 'id'=>'form')) !!}
        @endif
        <div class="form-group required {{ $errors->has('customer_id') ? 'has-error' : '' }}">
            {!! Form::label('customer_id', trans('invoice.customer'), array('class' => 'control-label required')) !!}
            <div class="controls">
                {!! Form::select('customer_id', $customers, (isset($invoice->customer_id)?$invoice->customer_id:null), array('id'=>'customer_id','class' => 'form-control')) !!}
        ...
        @if(!isset($invoice))
            <div class="form-group required {{ $errors->has('qtemplate_id') ? 'has-error' : '' }}">
                {!! Form::label('qtemplate_id', trans('invoice.quotation_template'), array('class' => 'control-label')) !!}
                <div class="controls">
                    {!! Form::select('qtemplate_id', $qtemplates, null, array('id'=>'qtemplate_id','class' => 'form-control')) !!}
        ...
        <div class="form-group required {{ $errors->has('invoice_date') ? 'has-error' : '' }}">
            {!! Form::label('invoice_date', trans('invoice.invoice_date'), array('class' => 'control-label required')) !!}
            <div class="controls">
                {!! Form::text('invoice_date', null, array('class' => 'form-control date')) !!}
                <span class="help-block">{{ $errors->first('invoice_date', ':message') }}</span>
            </div>
        </div>
        <div class="form-group required {{ $errors->has('due_date') ? 'has-error' : '' }}">
            {!! Form::label('due_date', trans('invoice.due_date'), array('class' => 'control-label required')) !!}
            <div class="controls">
                {!! Form::text('due_date', null, array('class' => 'form-control date')) !!}
                <span class="help-block">{{ $errors->first('due_date', ':message') }}</span>
            </div>
        </div>
        <div class="form-group required {{ $errors->has('payment_term') ? 'has-error' : '' }}">
            {!! Form::label('payment_term', trans('invoice.payment_term'), array('class' => 'control-label required')) !!}
            <div class="controls">
                ...
                <select name="payment_term" class="form-control">
                    ...
                    <option value="{{Settings::get('payment_term1')}}"
                            @if(isset($invoice) && Settings::get('payment_term1') == $invoice->payment_term) selected @endif>{{Settings::get('payment_term1')}} {{trans('invoice.days')}}</option>
                    @endif
                    @if(Settings::get('payment_term2')!='0')
                        <option value="{{Settings::get('payment_term2')}}"
                                @if(isset($invoice) && Settings::get('payment_term2') == $invoice->payment_term) selected @endif>{{Settings::get('payment_term2')}} {{trans('invoice.days')}}</option>
                    @endif
                    @if(Settings::get('payment_term3')!='0')
                        <option value="{{Settings::get('payment_term3')}}"
                                @if(isset($invoice) && Settings::get('payment_term3') == $invoice->payment_term) selected @endif>{{Settings::get('payment_term3')}} {{trans('invoice.days')}}</option>
                    @endif
                    <option value="0"
        ...
        <div class="form-group required {{ $errors->has('grootboekrekening') ? 'has-error' : '' }}">
            {!! Form::label('grootboekrekening', trans('grootboekrekening'), array('class' => 'control-label')) !!}
            <div class="controls">
                <select name="grootboekrekening" class="form-control">
                    <option value=""></option>
                    @foreach ($grootboekrekening as $item)
                        <option value="{{ $item }}"
                        @if(isset($invoice))
                        @foreach($getResultGrootboekrekening as $eacharr)
                			@if($eacharr['invoiceID'] == $invoice->id && $eacharr['soapGrootboekrekeningID'] == $item)
                            	selected="selected"
                            @endif
                		@endforeach
                        @endif>{{ $item }}</option>
        ...
        <div class="form-group required {{ $errors->has('sales_team_id') ? 'has-error' : '' }}">
            {!! Form::label('sales_team_id', trans('invoice.sales_team_id'), array('class' => 'control-label required')) !!}
            <div class="controls">
                {!! Form::select('sales_team_id', $salesteams, (isset($invoice)?$invoice->sales_team_id:null), array('id'=>'sales_team_id','class' => 'form-control')) !!}
        ...
        <div class="form-group required {{ $errors->has('sales_person_id') ? 'has-error' : '' }}">
            {!! Form::label('sales_person_id', trans('invoice.salesperson_id'), array('class' => 'control-label required')) !!}
            <div class="controls">
                {!! Form::select('sales_person_id', $staffs, null, array('id'=>'sales_person_id','class' => 'form-control')) !!}
                <span class="help-block">{{ $errors->first('sales_person_id', ':message') }}</span>
            </div>
        </div>
        <div class="form-group required {{ $errors->has('status') ? 'has-error' : '' }}">
            {!! Form::label('status', trans('invoice.status'), array('class' => 'control-label required')) !!}
            <div class="controls">
                {!! Form::select('status', $statuses, (isset($invoice)?$invoice->status:null), array('id'=>'status','class' => 'form-control')) !!}
                <span class="help-block">{{ $errors->first('status', ':message') }}</span>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12">
                <label class="control-label required">{{trans('invoice.products')}}
        ...
        <div class="form-group required {{ $errors->has('tax_amount') ? 'has-error' : '' }}">
            {!! Form::label('tax_amount', trans('invoice.taxes').' ('.floatval(Settings::get('sales_tax')).'%)', array('class' => 'control-label')) !!}
            <div class="controls">
        ...
        <div class="form-group required {{ $errors->has('grand_total') ? 'has-error' : '' }}">
            {!! Form::label('grand_total', trans('invoice.total'), array('class' => 'control-label')) !!}
            <div class="controls">
        ...
        <div class="form-group required {{ $errors->has('discount') ? 'has-error' : '' }}">
            {!! Form::label('discount', trans('invoice.discount').' (%)', array('class' => 'control-label')) !!}
            <div class="controls">
        ...
        <div class="form-group required {{ $errors->has('final_price') ? 'has-error' : '' }}">
            {!! Form::label('final_price', trans('invoice.final_price'), array('class' => 'control-label')) !!}
            <div class="controls">
        ...
        $(function () {
            ...
            $('#qtemplate_id').change(function () {
                if ($(this).val() > 0) {
                    $.ajax({
                        type: "GET",
                        url: '{{url("invoice/ajax_qtemplates_products")}}/' + $(this).val(),
