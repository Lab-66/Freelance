...
class MailboxController extends UserController
{
	...
	{
		//die('create');
		$title = trans('New Email Template');


        return view('user.mailbox.create', compact('title'));
	}
	public function store(Request $request)
	{
		
		
		$file = $request->file('af');
      $destinationPath = '/var/www/html/files/public/uploads/mail';
    ...
    public function getAllData()
    {
        ...
        $email_templates = ["0" => trans('mailbox.etemplate_select')] +
            $this->emailTemplateRepository->getAll()->get()
                ->map(function($email){
                    return [
                        'id' => $email->id,
                        'text' => $email->title,
    ...
    function sendEmail(MailboxRequest $request)
    {
        ...
        if (!empty($request->recipients)) {
            ...
            foreach ($request->recipients as $item) {
                if ($item != "0" && $item != "") {
					$userDatas = DB::table('users')->where('id', '=', $item)->get();
					$msg = $request->message;
					foreach ($positions as $value) {
					 $sub = substr($request->message, $value+strlen('{user.'),strlen($request->message));
						$data = substr($sub,0,strpos($sub,'}'));
						$msg = str_replace($data,$userDatas[0]->$data,$msg);
					 }
					$newMsg = str_replace('{user.','',str_replace('}','',$msg));
					
					//die;
                    $email = new Email($request->except('recipients','emailTemplate'));
					$email->message = $newMsg;
                    $email->to = $item;
